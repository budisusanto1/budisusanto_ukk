...
class Transaksi extends Model
{
    ...
    public function updateWithBiaya($id_parkir, $tarif_per_jam)
    {
        ...
        // Hitung durasi dalam jam (dengan pembulatan yang wajar)
        $durasi_jam = $interval->h + ($interval->i / 60);
        // Jika kurang dari 1 jam, cek apakah lebih dari 30 menit
        if ($durasi_jam < 1) {
            // Jika lebih dari 30 menit (0.5 jam), bulatkan ke 1 jam
            if ($interval->i >= 30) {
                $durasi_jam = 1;
            } else {
                // Jika kurang dari 30 menit, anggap 0 jam (gratis)
                $durasi_jam = 0;
            }
        }
        
        // Ambil jenis kendaraan untuk menentukan tarif
        $this->select('tb_kendaraan.jenis_kendaraan');
        $this->join('tb_kendaraan', 'tb_kendaraan.id_kendaraan = tb_transaksi.id_kendaraan');
        $transaksi_with_kendaraan = $this->find($id_parkir);
        $jenis_kendaraan = $transaksi_with_kendaraan['jenis_kendaraan'] ?? 'mobil';
        
        // Hitung biaya total dengan tarif progresif berdasarkan jenis kendaraan
        if ($jenis_kendaraan === 'motor') {
            // Tarif motor: Rp 1.000 per jam
            $biaya_total = $durasi_jam * 1000;
        } else {
            // Tarif mobil: Rp 1.000 per jam (sama dengan motor)
            $biaya_total = $durasi_jam * 1000;
        }
        ...
        log_message('debug', "UpdateWithBiaya - ID: $id_parkir, Jenis: $jenis_kendaraan, Durasi: $durasi_jam, Biaya: $biaya_total");
